...
import createScatterPlot, {
  Datum as ScatterPlotDatum,
} from './dataViz/scatterPlot';
import createBarChart, {
  Datum as BarChartDatum,
  LabelPlacement,
} from './dataViz/barChart';
import createClusterBarChart, {
  Datum as ClusterBarChartDatum,
} from './dataViz/clusterBarChart';
...
import createStackChart, {
  Datum as StackChartDatum,
  StackChartConfig,
} from './dataViz/stackChart';
import createClusterChart, {
  Datum as ClusterChartDatum,
} from './dataViz/clusterChart';
import createBoxAndWhiskersChart, {
  Datum as BoxAndWhiskersChartDatum,
} from './dataViz/boxAndWhiskers';
...
export const DataViz = (props: Props) => {
  ...
  const handleDownloadImage = (fileFormat: FileFormat) => {
    ...
    if (props.vizType === VizType.RadarChart && sizingNode) {
      ...
      width =
        sizingNode && sizingNode.clientWidth
          ? sizingNode.clientWidth * highResMultiplier
          : undefined;
      height =
        sizingNode && sizingNode.clientHeight
          ? sizingNode.clientHeight * highResMultiplier
          : undefined;
    }
  ...
  const downloadPNGButton =
    enablePNGDownload !== true ? null : (
      <DownloadImageButton onClick={() => handleDownloadImage(FileFormat.PNG)}>
        Download PNG
      </DownloadImageButton>
    );
  const downloadSVGButton =
    enableSVGDownload !== true ? null : (
      <DownloadImageButton onClick={() => handleDownloadImage(FileFormat.SVG)}>
        Download SVG
      </DownloadImageButton>
    );
  ...
  const downloadButtons =
    ...
    downloadDataButton !== null ? (
      <DownloadButtonsContainer
        style={{
          marginTop: props.vizType !== VizType.RadarChart ? '1rem' : undefined,
        }}
      >
        {downloadPNGButton}
        {downloadSVGButton}
    ...
    return (
      <Root style={rootStyles}>
        <SizingElm ref={sizingNodeRef} style={{ height: props.height }}>
          <svg
            ref={svgNodeRef}
            key={id + windowWidth + 'svg'}
            className={'react-fast-charts-root-svg'}
          />
        </SizingElm>
        {caption}
        {downloadButtons}
