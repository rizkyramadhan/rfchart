...
export default (input: Input) => {
  ...
  g.append('g')
    .style('font-family', labelFont ? labelFont : "'Source Sans Pro',sans-serif")
    .call(d3.axisLeft(yScale).tickFormat(formatNumber as any));
  ...
  function updateChart() {
    ...
    const extent = (d3 as any).event.selection;
    ...
    if (!extent) {
      ...
      areaChart.select('.brush').call((brush as any).move, null); // This remove the grey brush area as soon as the selection has been done
    }
